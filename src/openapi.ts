export const openapi = {
  ...
  paths: {
    ...
    '/v1/github/sync': { post: { security: [{ bearerAuth: [] }], responses: { '202': { description: 'Repository reconciliation requested' }, '401': { description: 'GitHub App or desktop authentication failed' }, '403': { description: 'GitHub App access was denied or rate-limited' } } } },
    '/v1/repositories': { get: { security: [{ bearerAuth: [] }], parameters: [{ name: 'cursor', in: 'query' }, { name: 'limit', in: 'query' }], responses: { '200': { description: 'Read-only cursor-paginated { items, next_cursor }' }, '401': { description: 'GitHub App or desktop authentication failed' }, '500': { description: 'Unexpected service error' } } } },
    ...
    '/v1/issues/{id}/comments': { get: { security: [{ bearerAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true }, { name: 'limit', in: 'query' }, { name: 'since', in: 'query' }], responses: { '200': { description: 'Issue comments received via signed GitHub webhooks' }, '401': { description: 'Missing or invalid desktop bearer token' }, '404': { description: 'Active issue not found' } } } },
